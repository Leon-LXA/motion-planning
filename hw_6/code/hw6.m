...
%% display the trajectory and corridor
...
for k = 1:n_seg
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            ...
            x_pos(idx) = x_pos(idx)+basis_p * poly_coef_x((k-1)*8+i+1,1);
            y_pos(idx) = y_pos(idx)+basis_p * poly_coef_y((k-1)*8+i+1,1);
        end
        idx = idx + 1;
    end
end
...
hold on;
scatter(x_pos(1,1:size(x_pos,2)),y_pos(1,1:size(y_pos,2)));
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    %                                   ...,
    %                                   pn_min, pn_max ];
    corridor_range = zeros(n_seg,4);
    for k = 1:n_seg
        corridor_range(k,1:4) = [(corridor(1,k)-corridor(3,k)),(corridor(1,k)+corridor(3,k)),(corridor(2,k)-corridor(4,k)),(corridor(2,k)+corridor(4,k))];
        
    end
    
    % STEP 3.2: get Aieq and bieq
    [Aieq, bieq] = getAbieq(n_seg, n_order, corridor_range, ts, v_max, a_max,axis);
